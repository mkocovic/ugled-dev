...
export default function SobnaDoorsPage() {
  const { title, subtitle, heroImage, features, types, gallery, technicalSpecifications, cta } = sobnaConfig;
        ...
        {features && (
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-24">
            {features.map((feature: any, index: number) => {
              const IconComponent = featureIconMapping[feature.icon as SobnaFeatureIconKey];
              return (
                <div
                  key={index}
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="space-y-4">
                {technicalSpecifications.left.map((spec: any, index: number) => (
                  <div key={index} className="p-4 border specs-border">
